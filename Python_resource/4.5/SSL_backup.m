...
%%
clc
sprintf('(%d, %f)',[nu_loop', store_error(:,1)]')
sprintf('(%d, %f)',[nu_loop', store_error(:,2)]')
sprintf('(%d, %f)',[nu_loop', store_error(:,3)]')
...
% %% Performance of RMT-improved versus standard Laplacian
% close all; clear; clc
% 
% cs = [1/2 1/2];
% k = length(cs); % nb of classes
...
% 
% nu_loop = 50:50:400;
% store_error = zeros(length(nu_loop),2);
% 
% iter = 1;
% nb_average_loop = 10;
% rng(15);
...
%     vec_C = -vec_C1 + vec_C2;
%     
%     store_error_tmp = zeros(nb_average_loop,2);
%     
%     for average_index = 1:nb_average_loop
%         X=zeros(p,n);
%         S = zeros(n,k);
...
%         hat_Sl = P(nl)*S(label_ind,:);
%         
%         %alpha = 2*norm(hat_K_uu);
%         alpha = norm(hat_K_uu) + 25;
%         hat_Su = ((alpha*eye(nu) - hat_K_uu )\hat_K_ul)*hat_Sl;
%         S(unlabel_ind,:) = hat_Su;
%         
%         estim_vec_C1 = S(unlabel_ind,1)>S(unlabel_ind,2);
...
% plot(nu_loop,store_error(:,1), 'r')
% plot(nu_loop,store_error(:,2),'b')
% legend('RMT improved', 'Classical Laplacian', 'Interpreter', 'latex', 'Fontsize', 15)
